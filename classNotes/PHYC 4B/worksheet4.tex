...
\maketitle
\begin{enumerate}
\item Pictured below is a spherical conducting ball of radius \(a\) surrounded by a spherical non-conducting "thick" shell of inner radius \(b\) and outer radius \(c\).  The conducting ball has a total charge of \(Q_1\) and the non-conducting shell has a total charge of \(Q_2\) uniformly distributed throughout its volume (i.e., the volume charge density \(\rho\) for the shell is constant). \begin{center}
\includegraphics[width=.9\linewidth]{./img/4.1.png}
\end{center}
\begin{enumerate}
\item Determine the volume charge density \(\rho\) of the non-conducting shell.
\item Determine the electric field at a point a distance \(r\) from the center of the conducting ball. Consider separately each case: \(r < a\) , \(a < r \leq b\), \(b \leq r \leq c\), and \(r \leq c\).
\item If \(Q_2 = -2Q_1\), for which values of \(r\) is the electric field zero?
\end{enumerate}
\end{enumerate}
