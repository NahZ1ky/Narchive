...
\newpage
Question set:
1. Two charges lie along the x-axis, a $-8.0 \mu C$ charge at $x=0$ and a $-2.0 \mu C$ charge at $x=6.0cm$.
   1. Calculate the electric field at the point $(x, y)=(0, 8.0cm)$ (along the y-axis).
   2. Find all points (besides infinity) where the electric field is zero.
2. A charged rod is placed on the x-axis between $x=d$ and $x=d+L$. Its charge density is not uniform, and is given by $\lambda=cx$, where $c$ is a positive constant.
   1. What are the SI units of $c$?
   2. Calculate the total charge of the rod (hint: $Q= \int dq$).
   3. Calculate the electric field at $x=0$.
   4. Suppose $L=2d$. If the rod is replaced by a point charge with the same total charge (calculated in part b), where should the point charge be located in order to give the same electric field at $x=0$ as for the rod.
   5. A proton is placed at $x=0$ and is released from rest. Describe the trajectory of the proton. Is the acceleration of the proton constant?
