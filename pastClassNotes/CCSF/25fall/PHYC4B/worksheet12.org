...
1. \( 1 \ \mathrm{A} \) wire of total resistance \( 0.030 \ \Omega \) is bent into a triangular loop shown below, and is being pulled out of a region containing a \( 2.0 \mathrm{T} \) uniform magnetic field at a speed of \( 20 \ \mathrm{cm}/ \mathrm{s} \).
   #+ATTR_LATEX: :height 3cm
   #+CAPTION: An isosceles right triangle (leg \( 10 \ \mathrm{cm} \)) leaving a magnetic field
   #+LABEL: Figure 12.1
   [[./img/12.1.jpg]]
   1. Calculate the magnetic flux through the loop as a function of \( t \), where \( t \) is the time since the triangular loop first emerges from the field. (Hint: What is \( x(t) \)?)
   2. Calculate the current in the loop at \( t = 0.20 \ \mathrm{s} \). In which direction is the current? \\

2. A close-packed solenoid of length \( 1.60 \ \mathrm{m} \) and diameter \( 3.60 \ \mathrm{cm} \) has a self-inductance of \( 23.0 \ \mathrm{mH} \). The solenoid is constructed from copper wiring which has a resistance of \( 0.0200 \ \Omega\) for each meter of wiring. The solenoid is then connected to a \( 6.00 \ \mathrm{V} \) battery which has an internal resistance of \( 4.00 \ \Omega \). This is depicted below.
   #+ATTR_LATEX: :height 3cm
   #+CAPTION: Solenoid connected to battery
   #+LABEL: Figure 12.2
   [[./img/12.2.jpg]]
   1. Calculate the number of turns in the solenoid.
   2. Calculate the time constant of the LR circuit above. Don't forget to account for the resistance of the solenoid wiring.
   3. How much time is required after the switch is closed for the current in the circuit above to reach \( 99.0 \% \) of its final value?
